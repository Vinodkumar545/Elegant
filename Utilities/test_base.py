...
from Utilities import object_property as OP
...
def activate_driver(brower_name, headless=False):
	...
	driver = webdriver.Remote(command_executor=settings.NODE_URL, desired_capabilities=desired_capabilities)
...
def get_text(driver, web_element):
	try:
		...
		LOGGER.exception("get_text  Exception: %s" %(e))
